%%Maral Kasiri
%%Gerarld Loeb Muscle Model
%reference: Cheng J. et al 2000
%------------------------------------
function ModelOutput= GerLoeb(Input,Ueff)
%-----------------------------
%Input:
%Output:
%-----------------------------
...
t = 0:1/Fs:(length(Ueff)-1)/Fs;

%%
% muscle architectural parameters

alpha = Input.pennationAngle; % pennation angle
mass = Input.mass; % muscle mass
Lm_initial = Input.muscleInitialLength; % muscle initial length
Lt_initial = Input.tendonInitialLength; % tendon initial length
Lmt = Lm_initial*cos(alpha)+Lt_initial; % intial musculotendon length
L0 = Input.optimalLength; % optimal muscle length
L_tendon = Input.tendonLength; % tendon slack length
L0T = L_tendon*1.05; % optimal tendon length
%%
% calculate maximal force output of the muscle based on PCSA
density = 1.06; % g/cm^3
PCSA = (mass*1000)/density/Input.optimalLength; %physiological cross-sectional area
Specific_Tension = 31.4;
F0 = PCSA * Specific_Tension; % maximal force
Fmax = Input.MVIC; % maximal voluntary force determined empilically

%%
% calculate initial length based on balance between stiffness of muscle and
% tendon
[Lce,Lse,Lmax] =  InitialLength(Input);
...
OutputForceTendon = zeros(1,length(Ueff));
OutputForceMuscle = zeros(1,length(Ueff));
OutputLse = zeros(1,length(Ueff));   
OutputLce = zeros(1,length(Ueff));
OutputVce = zeros(1,length(Ueff));
OutputAce = zeros(1,length(Ueff));
OutputAf_slow = zeros(1,length(Ueff));
OutputAf_fast = zeros(1,length(Ueff));
MuscleAcceleration = zeros(1,length(Ueff));
MuscleVelocity = zeros(1,length(Ueff));
MuscleLength = zeros(1,length(Ueff));
...
OutputVce = zeros(1,length(Ueff));
OutputAce = zeros(1,length(Ueff));
OutputUeff = zeros(1,length(Ueff));
OutputAf_slow = zeros(1,length(Ueff));
OutputAf_fast = zeros(1,length(Ueff));
MuscleAcceleration = zeros(1,length(Ueff));
MuscleVelocity = zeros(1,length(Ueff));
...
%%
    ...
    ForceTotal = forceContractile(Lce,Vce,Lmax,Ueff(1));
    ...
    ForceTotal = forceContractile(Lce,Vce,Lmax,Ueff(i));
    ForceTotal = ForceTotal*F0;
    ...
    OutputUeff(i) = Ueff(i); % effective neural drive to muscle
...
ModelOutput.Force = OutputForceMuscle; % muscle force
ModelOutput.ForceTendon = OutputForceTendon; % tendon force
ModelOutput.Lce = OutputLce; % muscle length 
ModelOutput.Vce = OutputVce; % muscle velocity
ModelOutput.Ace = OutputAce; % muscle acceleration
ModelOutput.Lse = OutputLse; % tendon length
ModelOutput.U = Ueff; % effective neural drive
ModelOutput.Af_slow = OutputAf_slow; % activation-frequency relationship for slow-twitch fiber
ModelOutput.Af_fast = OutputAf_fast; % activation-frequency relationship for fast-twitch fiber
